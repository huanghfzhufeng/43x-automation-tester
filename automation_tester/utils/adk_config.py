...
def before_model_callback(
    ...
    try:
        ...
        # 从第一条 user 消息开始保留
        filtered_events = session.events[cut_idx:] if cut_idx is not None else session.events
        ...
        filtered_events = [
            ...
            if (
                ...
                and not (
                    bool(e.get_function_calls()) or bool(e.get_function_responses())
                )  # 移除工具调用
            )
        ]
        ...
        llm_request.contents = _get_contents(
            invocation_context.branch, filtered_events, callback_context.agent_name
        )
                ...
                if rag_service:
                    ...
                    if results:
                        ...
                        for i, result in enumerate(results):
                            relevant_materials.append(f"[相关材料 {i + 1}]\n{result.chunk}")
