        ...
        try:
            ...
            try:
                ...
                with pdfplumber.open(self._path) as pdf:
                    ...
                    logger.info(f"PDF 解析完成: {pages_with_text}/{total_pages} 页有文本，共 {total_chars} 字符")
                    
                    # 如果没有提取到任何文本，给出警告
                    if pages_with_text == 0:
                        logger.warning(f"PDF 文件没有可提取的文本内容，可能是图片型 PDF（扫描件）")
            ...
            raise RuntimeError(f"解析PDF文件 {self._path} 失败: {e}")
