...
def add_to_cache(session_id: str, agent: Any):
    ...
    if session_id in session_cache:
        ...
        logger.info(f"➕ 添加到缓存: {session_id} (缓存大小: {len(session_cache)}/{MAX_CACHE_SIZE})")
...
async def start_test(request: StartTestRequest):
    ...
    with LogContext(logger, f"启动测试 - {scenario_name}"):
        try:
            ...
            logger.info(f"📊 当前缓存状态: {cache_stats_info['size']}/{cache_stats_info['max_size']} "
...
async def get_answer(request: AnswerRequest):
    ...
    with LogContext(logger, f"处理问题 - {request.session_id[:16]}..."):
        try:
            ...
            logger.info(f"   问题是否为空: {not request.question or request.question.strip() == ''}")
            ...
            if agent is None:
                ...
                # 记录缓存状态
                cache_stats_info = get_cache_stats()
                logger.error(f"📊 缓存状态: {cache_stats_info['size']}/{cache_stats_info['max_size']} "
                           f"(命中率: {cache_stats_info['hit_rate']})")
            ...
            # 记录缓存状态
            cache_stats_info = get_cache_stats()
            logger.debug(f"📊 缓存状态: {cache_stats_info['size']}/{cache_stats_info['max_size']} "
                        f"(命中率: {cache_stats_info['hit_rate']})")
